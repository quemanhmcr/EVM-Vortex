...
describe('StakingManager', function () {
  ...
  describe('Staking', function () {
    ...
    it('Should revert if staking amount is zero', async function () {
      const { stakingManager, owner } = await loadFixture(deployStakingManagerFixture)
      await expect(stakingManager.connect(owner).stake({ value: 0 })).to.be.revertedWith(
        'StakingManager: Cannot stake 0',
      )
    })
  })
  ...
  describe('Unstaking', function () {
    ...
    it('Should revert if user tries to unstake more than they have', async function () {
      ...
      await expect(stakingManager.connect(owner).unstake(unstakeAmount)).to.be.revertedWith(
        'StakingManager: Insufficient stake',
      )
    })
    ...
    it('Should revert if unstaking amount is zero', async function () {
      ...
      await expect(stakingManager.connect(owner).unstake(0)).to.be.revertedWith(
        'StakingManager: Cannot unstake 0',
      )
    })
  })
})
