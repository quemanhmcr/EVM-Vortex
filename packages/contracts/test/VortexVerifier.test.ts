...
import { StakingManager, VortexVerifier } from "../typechain-types";
...
describe("VortexVerifier", function () {
  ...
  async function deployContractsFixture() {
    ...
    const vortexVerifier = await VortexVerifierFactory.deploy(await stakingManager.getAddress(), CHALLENGE_PERIOD);
  ...
  async function proposeAndGetDataId(vortexVerifier: VortexVerifier, proposer: HardhatEthersSigner, payload: { timestamp: number; data: Uint8Array; }) {
      const tx = await vortexVerifier.connect(proposer).proposeData(payload);
      const receipt = await tx.wait();
      const event = receipt?.logs.find((e: any) => e.eventName === 'DataProposed');
      return event?.args[0];
  }
  ...
  describe("Propose Data (Happy Path)", function () {
    it("Should revert if proposeData is called by a non-staker", async function () {
      ...
      await expect(vortexVerifier.connect(nonStaker).proposeData(payload)).to.be.revertedWith("VortexVerifier: Caller is not a staker");
    });
    ...
    it("Should allow a valid staker to propose data and emit an event", async function () {
      ...
      await expect(vortexVerifier.connect(proposer).proposeData(payload)).to.emit(vortexVerifier, "DataProposed");
    });
  ...
  describe("Execute Intent (Happy Path)", function () {
    it("Should revert if executeIntent is called within the challenge period", async function () {
        ...
        await expect(vortexVerifier.executeIntent(dataId)).to.be.revertedWith("VortexVerifier: Challenge period has not ended");
    });
    ...
    it("Should allow executeIntent after the challenge period", async function () {
        ...
        await expect(vortexVerifier.executeIntent(dataId)).to.emit(vortexVerifier, "DataExecuted").withArgs(dataId);
    ...
    it("Should revert if intent is already executed", async function () {
        ...
        await expect(vortexVerifier.executeIntent(dataId)).to.be.revertedWith("VortexVerifier: Intent already executed");
    });
    ...
    it("Should revert if proposal does not exist", async function () {
        ...
        await expect(vortexVerifier.executeIntent(fakeDataId)).to.be.revertedWith("VortexVerifier: Proposal does not exist");
    });
  });
});