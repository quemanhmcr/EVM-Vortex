...
const { RPC_URL, PROPOSER_PRIVATE_KEY, VORTEX_VERIFIER_ADDRESS, PROPOSE_INTERVAL_MS } = process.env

if (!RPC_URL || !PROPOSER_PRIVATE_KEY || !VORTEX_VERIFIER_ADDRESS || !PROPOSE_INTERVAL_MS) {
  console.error(
    'FATAL: One or more environment variables are missing. Please check your .env file or the script environment.',
  )
  process.exit(1)
}
...
async function main() {
  ...
  const vortexVerifierContract = new Contract(
